...
app.get('/produto', async (req, res) => {
  try {
    const cursos = await knex('produto').select('*');
...
app.get('/produto/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const curso = await knex('produto').where('id', id).first();
...
app.post('/produto', async (req, res) => {
  try {
    ...
    const [id] = await knex('produto').insert({ nome, quantidade, preco });
...
app.delete('/produto/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const deletado = await knex('produto').where('id', id).del();
...
app.put('/produto/:id', async (req, res) => {
  try {
    ...
    const atualizado = await knex('produto').where('id', id).update(dadosParaAtualizar);
